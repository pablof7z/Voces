  ...
  {#if isOpen}
    <div class="absolute left-0 mt-1 w-64 bg-neutral-900 border border-neutral-800 rounded-lg shadow-xl z-50 overflow-hidden">
      ...
      {#if allPacks.length > 0}
        ...
        <div class="px-2 py-1">
          <div class="text-xs text-neutral-500 px-2 py-1 font-medium">Follow Packs</div>
          {#each allPacks as pack (pack.id)}
            ...
            <button
              ...
              class="w-full px-3 py-2.5 rounded-lg hover:bg-neutral-800 transition-colors text-left flex items-center gap-3 {isSelected ? 'bg-neutral-800/50' : ''}"
            >
              {#if packImage}
