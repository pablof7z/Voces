...
<div class="bg-black border border-neutral-800 rounded-xl p-6">
  <div class="flex items-center justify-between mb-4">
    <h3 class="text-lg font-semibold text-white">Nutzap Monitor</h3>
    <button
  ...
  <div class="grid grid-cols-3 gap-3 mb-4">
    <div class="bg-neutral-900 border border-neutral-700 rounded-lg p-3 text-center">
    ...
    <div class="bg-neutral-900 border border-neutral-700 rounded-lg p-3 text-center">
      ...
      <div class="text-xs text-neutral-400 mt-1">Redeemed</div>
    </div>
    <div class="bg-neutral-900 border border-neutral-700 rounded-lg p-3 text-center">
      <div class="text-2xl font-bold text-red-500">{failedCount}</div>
      <div class="text-xs text-neutral-400 mt-1">Failed</div>
    </div>
  </div>
        ...
        <div>
          ...
          <div class="space-y-2">
            {#each nutzaps.pending as nutzap (nutzap.id)}
              <div class="p-2 bg-neutral-900 border border-neutral-700 rounded text-sm">
                <div class="text-white">{nutzap.id.slice(0, 16)}...</div>
                <div class="text-neutral-400 text-xs">{formatDate(nutzap.created_at || 0)}</div>
              </div>
            {/each}
          </div>
        </div>
