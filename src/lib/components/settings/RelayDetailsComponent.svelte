...
<div class="flex-1 min-w-0">
  <div class="flex flex-wrap items-center gap-2">
    ...
    <!-- URL -->
    <span class="font-mono text-xs md:text-sm text-neutral-900 dark:text-neutral-100 break-all">
      {relay.url}
    </span>
    ...
    <div class="mt-2 space-y-1">
      {#if info.name}
        <div class="flex items-start gap-2">
          <span class="text-xs font-medium text-neutral-500 dark:text-neutral-400 min-w-[60px]">Name:</span>
          <span class="text-sm font-semibold text-neutral-900 dark:text-neutral-100">{info.name}</span>
        </div>
      {/if}

      {#if info.description}
        <div class="flex items-start gap-2">
          <span class="text-xs font-medium text-neutral-500 dark:text-neutral-400 min-w-[60px]">About:</span>
          <span class="text-sm text-neutral-700 dark:text-neutral-300">{info.description}</span>
        </div>
      {/if}

      {#if info.software || info.version}
        <div class="flex items-start gap-2">
          <span class="text-xs font-medium text-neutral-500 dark:text-neutral-400 min-w-[60px]">Software:</span>
      ...
      {#if info.relay_countries && info.relay_countries.length > 0}
        <div class="flex items-start gap-2">
          <svg class="w-3 h-3 text-neutral-400 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="flex flex-wrap gap-2 mt-2">
        ...
        {/if}
        {#if info.contact}
          <span class="text-xs bg-neutral-100 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-400 px-2 py-0.5 rounded-full">
            📧 {info.contact}
          </span>
        {/if}
      </div>
