<script lang="ts">
  ...
  function handleThemeColorChange(newColor: 'red' | 'cyan' | 'yellow' | 'lime') {
...
<div class="space-y-6">
  <div>
    <h2 class="text-lg font-semibold text-neutral-900 dark:text-foreground mb-4">
  ...
  <div class="border-b border pb-6">
    <div class="flex items-center gap-3 mb-4">
      ...
      <h3 class="text-base font-medium text-neutral-900 dark:text-foreground">
      ...
      {#each colorOptions as option}
        <button
          ...
          class="px-4 py-3 rounded-lg border transition-all {settings.themeColor === option.id
            ? 'border-2'
            : 'bg-white dark:bg-background border hover:bg-neutral-50 dark:hover:bg-card'}"
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <div class="flex flex-col items-start">
              <span class="font-medium text-neutral-900 dark:text-foreground">{option.name}</span>
              <span class="text-xs text-muted-foreground dark:text-muted-foreground">{option.description}</span>
            </div>
          </div>
        </button>
      {/each}
