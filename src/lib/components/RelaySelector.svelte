<script lang="ts">
  ...
  const displayName = $derived.by(() => {
    if (relayFilter.selectedRelay && selectedRelayInfo?.info?.name) {
      return selectedRelayInfo.info.name;
    }
    if (relayFilter.selectedRelay) {
      return relayFilter.selectedRelay.replace('wss://', '').replace('ws://', '');
    }
    return 'Following';
  });
...
<div class="relative" use:clickOutside={handleClickOutside}>
  ...
  <button
    onclick={handleClick}
    class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors w-full {active ? 'text-orange-500 bg-orange-500/10' : 'text-neutral-300 hover:bg-neutral-800/50'}"
  >
  ...
  {#if isOpen && active}
    <div class="absolute left-0 mt-1 w-full bg-neutral-900 border border-neutral-800 rounded-lg shadow-xl z-50 overflow-hidden">
      <!-- Following option -->
      <button
