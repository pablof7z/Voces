...
<div class="bg-white dark:bg-background rounded-lg md:rounded-xl border border p-3 md:p-4 hover:shadow-lg transition-shadow">
    ...
    <div class="flex items-center gap-3">
      ...
      <div>
        <div class="flex items-center gap-2">
          <h3 class="font-semibold text-sm md:text-base text-neutral-900 dark:text-foreground">
  ...
  <div class="grid grid-cols-3 gap-2 md:gap-4 mb-3 md:mb-4">
    <div>
      <p class="text-xs text-muted-foreground dark:text-muted-foreground mb-0.5 md:mb-1">Amount</p>
      <div class="flex items-center gap-1 md:gap-2">
        ...
        <span class="font-mono font-semibold text-xs md:text-base text-neutral-900 dark:text-foreground">
          {(order.satsAmount / 100000000).toFixed(4)}
        </span>
      </div>
      <p class="text-xs text-muted-foreground dark:text-muted-foreground mt-0.5 md:mt-1 hidden md:block">
        {order.satsAmount.toLocaleString()} sats
      </p>
    </div>

    <div>
      <p class="text-xs text-muted-foreground dark:text-muted-foreground mb-0.5 md:mb-1">Price</p>
      <div class="flex items-center gap-1">
        <span class="text-sm md:text-base hidden md:inline">{currencyInfo.flag}</span>
        <p class="text-sm md:text-lg font-semibold text-neutral-900 dark:text-foreground">
    ...
    <div class="min-w-0">
      <p class="text-xs text-muted-foreground dark:text-muted-foreground mb-0.5 md:mb-1">Payment</p>
      <div class="flex items-center gap-1 md:gap-2">
        <span class="text-sm md:text-lg flex-shrink-0">{paymentInfo.icon}</span>
        <div class="flex flex-col min-w-0">
          <span class="font-medium text-xs md:text-base text-neutral-900 dark:text-foreground truncate max-w-[80px] md:max-w-none">
            {order.paymentMethod}
          </span>
          {#if paymentInfo.region}
            <span class="text-xs text-muted-foreground dark:text-muted-foreground hidden md:inline">
              {paymentInfo.region}
            </span>
          {/if}
        </div>
  ...
  <div class="flex items-center gap-2">
    <button
      ...
      class="flex-1 flex items-center justify-center gap-1 md:gap-2 px-3 md:px-4 py-1.5 md:py-2 bg-orange-600 text-foreground rounded-lg hover:bg-orange-700 transition-colors text-sm md:text-base"
    >
    ...
    <button class="p-1.5 md:p-2 border border rounded-lg hover:bg-neutral-50 dark:hover:bg-card transition-colors">
      <svg class="w-3 h-3 md:w-4 md:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
      </svg>
    </button>
    <button class="p-1.5 md:p-2 border border rounded-lg hover:bg-neutral-50 dark:hover:bg-card transition-colors hidden md:block">
      <svg class="w-3 h-3 md:w-4 md:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
