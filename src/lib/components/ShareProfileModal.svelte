...
{#if isOpen}
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onclick={onClose}>
    <div class="bg-neutral-900 rounded-2xl max-w-md w-full relative" onclick={(e) => e.stopPropagation()}>
      <button
        onclick={onClose}
        class="absolute top-4 right-4 p-2 hover:bg-neutral-800 rounded-full transition-colors"
      >
      ...
      <div class="p-6">
        ...
        <div class="bg-gradient-to-br from-orange-500 to-red-500 rounded-xl p-6 mb-6">
          <div class="flex flex-col items-center text-white">
            ...
            {#if profile?.nip05}
              <p class="text-white/80 text-sm">@{profile.nip05.split('@')[0]}</p>
            {/if}
          </div>
        </div>

        <div class="flex justify-center mb-6 p-4 bg-neutral-800 rounded-xl">
        ...
        <div class="space-y-3 mb-6">
          <button
            ...
            class="w-full flex items-center justify-between px-4 py-3 bg-neutral-800 hover:bg-neutral-700 rounded-lg transition-colors"
          >
        ...
        <div class="space-y-2">
          <p class="text-sm text-neutral-400 mb-3">Share on:</p>
          <div class="grid grid-cols-2 gap-3">
            <button
              onclick={() => shareOnPlatform('whatsapp')}
              class="flex items-center justify-center gap-2 px-4 py-3 bg-green-500 hover:bg-green-600 text-white rounded-lg transition-colors"
            >
            ...
            <button
              ...
              class="flex items-center justify-center gap-2 px-4 py-3 bg-black hover:bg-neutral-900 text-white rounded-lg transition-colors"
            >
              <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
          ...
          {#if typeof navigator !== 'undefined' && navigator.share}
            <button
              ...
              class="w-full flex items-center justify-center gap-2 px-4 py-3 bg-orange-600 hover:bg-orange-700 text-white rounded-lg transition-colors mt-3"
            >
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
